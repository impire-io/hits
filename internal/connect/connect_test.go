...
func TestEffectiveNameResolution(t *testing.T) {
	...
	// The nats CLI's selection marker is the last fallback.
	...
	if name, _ := effectiveName(""); name != "selected" {
		t.Fatalf("selected marker: got %q", name)
	}

	// The client config default outranks the marker.
...
func TestLookupCollisionIsAnError(t *testing.T) {
	cfgHome := setXDG(t)
	writeHitsContext(t, cfgHome, "both", map[string]any{"url": "nats://x:4222"})
	writeNatsContext(t, cfgHome, "both", map[string]any{"url": "nats://x:4222"})

	_, err := lookup("both")
	if err == nil || !strings.Contains(err.Error(), "defined twice") {
		t.Fatalf("want defined-twice error, got %v", err)
	}
}
...
func TestOAuthContextValidation(t *testing.T) {
	...
	writeHitsContext(t, cfgHome, "conflicted", map[string]any{
		"url": "nats://x:4222", "token": "static",
		"oauth": map[string]any{"issuer": "https://idp", "client_id": "c"},
	})
	if _, err := lookup("conflicted"); err == nil || !strings.Contains(err.Error(), "both oauth and a static token") {
		t.Fatalf("want oauth+token conflict, got %v", err)
	}

	writeHitsContext(t, cfgHome, "halfway", map[string]any{
		"url": "nats://x:4222", "oauth": map[string]any{"issuer": "https://idp"},
	})
	if _, err := lookup("halfway"); err == nil || !strings.Contains(err.Error(), "issuer and client_id") {
		t.Fatalf("want missing-field error, got %v", err)
	}

	writeHitsContext(t, cfgHome, "scoped", map[string]any{
		"url": "nats://x:4222", "oauth": map[string]any{"issuer": "https://idp", "client_id": "c"},
	})
	ref, err := lookup("scoped")
	if err != nil {
		t.Fatal(err)
	}
	if got := strings.Join(ref.hits.OAuth.Scopes, " "); got != "openid offline_access" {
		t.Fatalf("default scopes: got %q", got)
	}
}

// TestConnectParity: the same context file works identically from either
// directory — the nats context schema is the schema (FR-02).
func TestConnectParity(t *testing.T) {
	url := natstest.Start(t)
	body := map[string]any{"url": url}

	cfgHome := setXDG(t)
	writeHitsContext(t, cfgHome, "viahits", body)
	writeNatsContext(t, cfgHome, "vianats", body)

	for _, name := range []string{"viahits", "vianats"} {
...
func TestConnectOAuthFreshToken(t *testing.T) {
	...
	cfgHome := setXDG(t)
	writeHitsContext(t, cfgHome, "sso", map[string]any{"url": url, "oauth": oauthBlock(idp)})
...
func TestConnectOAuthNoCacheFailsFast(t *testing.T) {
	...
	cfgHome := setXDG(t)
	writeHitsContext(t, cfgHome, "sso", map[string]any{"url": "nats://127.0.0.1:1", "oauth": oauthBlock(idp)})
...
// make the spend exclusive (FR-08).
...
func TestAuthVerbsRequireOAuthContext(t *testing.T) {
	...
	writeHitsContext(t, cfgHome, "nooauth", map[string]any{"url": "nats://x:4222"})

	if _, err := Status("plain"); err == nil || !strings.Contains(err.Error(), "not a hits context") {
		t.Fatalf("nats context: got %v", err)
	}
